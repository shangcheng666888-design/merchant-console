...
export function buildDashboardInsight(input: DashboardInsightInput): string {
  ...
  if (pendingOrders > 0) {
    return tr(lang, {
      ...
      it: `${pendingOrders} ordine/i in attesa di evasione: dai priorità alle spedizioni per proteggere la salute del negozio.`,
      vi: `${pendingOrders} đơn hàng chờ giao — hãy ưu tiên xử lý để duy trì sức khỏe cửa hàng.`,
      fr: `${pendingOrders} commande(s) en attente d'expédition — priorisez l'expédition pour préserver la santé de la boutique.`,
    })
  }
  ...
  if (todayOrders === 0) {
    if (healthIndex >= 90) {
      return tr(lang, {
        ...
        es: 'Aún no hay pedidos hoy. La salud de la tienda es excelente: revisa tu plan de crecimiento para obtener más tráfico.',
        it: 'Nessun ordine oggi. La salute del negozio è ottima: consulta il piano di crescita per più traffico.',
        vi: 'Chưa có đơn hàng hôm nay. Sức khỏe cửa hàng rất tốt — hãy xem kế hoạch tăng trưởng để có thêm lưu lượng.',
        fr: 'Aucune commande aujourd\'hui. La santé de la boutique est excellente — consultez votre plan de croissance pour plus de trafic.',
      })
    }
    return tr(lang, {
      ...
      es: 'Aún no hay pedidos hoy: optimiza las fichas de producto y revisa tu plan de crecimiento.',
      it: 'Nessun ordine oggi: ottimizza le schede prodotto e rivedi il piano di crescita.',
      vi: 'Chưa có đơn hàng hôm nay — hãy tối ưu mô tả sản phẩm và xem lại kế hoạch tăng trưởng.',
      fr: 'Aucune commande aujourd\'hui — optimisez vos fiches produits et consultez votre plan de croissance.',
    })
  }
  ...
  if (todaySales > yesterdaySales && yesterdaySales > 0) {
    ...
    return tr(lang, {
      ...
      it: `Le vendite di oggi sono in aumento del ${pct}% rispetto a ieri: ottimo slancio. Considera di promuovere i prodotti più venduti.`,
      vi: `Doanh thu hôm nay tăng ${pct}% so với hôm qua — đà kinh doanh tốt. Hãy cân nhắc quảng bá sản phẩm bán chạy.`,
      fr: `Les ventes d'aujourd'hui sont en hausse de ${pct} % par rapport à hier — la dynamique est forte. Envisagez de promouvoir vos meilleures ventes.`,
    })
  }
  ...
  if (goodRate >= 95 && healthIndex >= 90) {
    return tr(lang, {
      ...
      es: 'La reputación y la salud de la tienda son excelentes: mantén la calidad del producto y la rapidez en los envíos.',
      it: 'Reputazione e salute del negozio sono eccellenti: mantieni qualità del prodotto e velocità di evasione.',
      vi: 'Uy tín và sức khỏe cửa hàng đều ở mức cao — hãy duy trì chất lượng sản phẩm và tốc độ giao hàng.',
      fr: 'La réputation et la santé de la boutique sont excellentes — maintenez la qualité des produits et la rapidité d\'expédition.',
    })
  }
  ...
  return tr(lang, {
    ...
    es: 'Las métricas son estables: mantén el catálogo actualizado y vigila el rendimiento de conversión.',
    it: 'Le metriche sono stabili: mantieni il catalogo aggiornato e monitora le conversioni.',
    vi: 'Chỉ số kinh doanh ổn định — hãy cập nhật sản phẩm và theo dõi hiệu suất chuyển đổi.',
    fr: 'Les indicateurs sont stables — maintenez votre catalogue à jour et suivez les performances de conversion.',
  })
}
