...
const MerchantWalletRecharge: React.FC = () => {
  ...
  const handleConfirm = async () => {
    ...
    if (!Number.isFinite(amountValue) || amountValue <= 0) {
      showToast(
        lang === 'zh' ? '请输入正确的金额' : 'Please enter a valid amount',
        'error',
      )
      return
    }
    if (!screenshotUrl) {
      showToast(
        lang === 'zh' ? '请上传交易截图' : 'Please upload transaction screenshot',
        'error',
      )
      return
    }
    try {
      ...
      if (!raw) {
        showToast(
          lang === 'zh' ? '请先登录店铺账号' : 'Please log in to your shop account',
          'error',
        )
        return
      }
        ...
        <div className="wallet-recharge-form merchant-wallet-recharge-form">
          <div className="merchant-wallet-recharge-grid">
            ...
            <div className="wallet-recharge-field merchant-wallet-recharge-qr-cell">
              <label className="wallet-recharge-label">
                {lang === 'zh' ? '充值二维码' : 'Recharge QR code'}
              </label>
            ...
            <div className="wallet-recharge-field merchant-wallet-recharge-address-cell">
              ...
              <div className="wallet-recharge-address-row">
                <input className="wallet-recharge-address-input" value={depositAddress || (lang === 'zh' ? '暂无收款地址' : 'No deposit address')} readOnly />
                <button
                  type="button"
